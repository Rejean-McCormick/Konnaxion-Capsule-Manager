...
$script:SshReachable = $false
$script:RemoteCopySucceeded = $false
$script:RemoteDiagnosticSucceeded = $false
...
if [ "$(id -u)" -ne 0 ]; then
  if command -v sudo >/dev/null 2>&1 && sudo -n true >/dev/null 2>&1; then
    ...
    KX_PRIV_MODE="unavailable"
  fi
fi

priv() {
  if [ "${#KX_SUDO[@]}" -gt 0 ]; then
    "${KX_SUDO[@]}" "$@"
  else
    "$@"
  fi
...
prun() {
  echo
  echo ">>> [privileged:${KX_PRIV_MODE}] $*"
  if [ "${#KX_SUDO[@]}" -gt 0 ]; then
    sudo -n bash -lc "$*" 2>&1 | redact
  else
    bash -lc "$*" 2>&1 | redact
  fi
  local rc=${PIPESTATUS[0]}
...
echo "Privilege mode: ${KX_PRIV_MODE}"
if [ "${KX_PRIV_MODE}" = "unavailable" ]; then
  echo "WARN: non-root diagnostic user has no passwordless sudo; protected runtime/Docker checks may be incomplete."
fi

if ! priv test -f "${COMPOSE_FILE}" 2>/dev/null; then
  DISCOVERED_COMPOSE="$(priv find "${KX_ROOT}/instances/${INSTANCE_ID}" -maxdepth 6 -type f -name docker-compose.runtime.yml -print -quit 2>/dev/null || true)"
  if [ -n "${DISCOVERED_COMPOSE}" ]; then
    echo "Compose fallback discovered: ${DISCOVERED_COMPOSE}"
    COMPOSE_FILE="${DISCOVERED_COMPOSE}"
  fi
fi
...
prun "ufw status verbose || true"
prun "iptables -S || true"
prun "iptables -t nat -S || true"
prun "nft list ruleset 2>/dev/null | head -300 || true"
...
for p in "${FORBIDDEN_PORTS[@]}"; do
  echo
  echo ">>> port ${p}"
  priv ss -ltnp "sport = :${p}" 2>&1 | redact
done

section "REMOTE 4. Konnaxion folder layout"
prun "ls -la ${KX_ROOT} || true"
prun "find ${KX_ROOT} -maxdepth 3 -type d -printf '%M %u:%g %p\n' 2>/dev/null | sort | head -300"
prun "find ${KX_ROOT} -maxdepth 4 -type f \( -name '*.kxcap' -o -name '*.yml' -o -name '*.yaml' -o -name '*.json' -o -name '*.env' -o -name '*.log' \) -printf '%s bytes %TY-%Tm-%Td %TH:%TM %p\n' 2>/dev/null | sort | head -400"
prun "stat ${CAPSULE_FILE} || true"
prun "ls -lah ${CAPSULE_DIR} || true"

section "REMOTE 5. Agent service and private health"
prun "systemctl status konnaxion-agent --no-pager -l || true"
prun "systemctl cat konnaxion-agent --no-pager || true"
prun "journalctl -u konnaxion-agent -n 260 --no-pager || true"
...
prun "ls -la ${MANAGER_DIR} || true"
prun "cd ${MANAGER_DIR} 2>/dev/null && pwd && git rev-parse HEAD 2>/dev/null && git status --short 2>/dev/null || true"
prun "cd ${MANAGER_DIR} 2>/dev/null && python3 --version && which python3 && which uv && uv --version || true"
prun "cd ${MANAGER_DIR} 2>/dev/null && test -f pyproject.toml && grep -nE 'kx-agent|kx-manager|kx-builder|kx =' pyproject.toml || true"
prun "cd ${MANAGER_DIR} 2>/dev/null && find . -maxdepth 2 -type f \( -name '*.log' -o -name '*.json' -o -name '*.env' \) -printf '%s bytes %TY-%Tm-%Td %TH:%TM %p\n' 2>/dev/null | sort | head -200 || true"

section "REMOTE 7. Docker daemon"
prun "systemctl status docker --no-pager -l || true"
prun "journalctl -u docker -n 160 --no-pager || true"
prun "docker version || true"
prun "docker info || true"
prun "docker system df || true"
prun "docker ps -a --no-trunc || true"
prun "docker images || true"
prun "docker volume ls || true"
prun "docker network ls || true"
prun "docker stats --no-stream || true"

section "REMOTE 8. Container restart / health summary"
prun 'for cid in $(docker ps -aq 2>/dev/null); do docker inspect --format "{{.Name}} status={{.State.Status}} running={{.State.Running}} restart={{.RestartCount}} exit={{.State.ExitCode}} oom={{.State.OOMKilled}} error={{.State.Error}} health={{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}} image={{.Config.Image}}" "$cid"; done'

section "REMOTE 9. Docker Compose runtime file"
if priv test -f "${COMPOSE_FILE}" 2>/dev/null; then
  echo "Compose file exists: ${COMPOSE_FILE}"
  prun "ls -lah ${COMPOSE_FILE}"
  prun "sed -n '1,260p' ${COMPOSE_FILE}"
  prun "docker compose -f ${COMPOSE_FILE} config --quiet"
  prun "docker compose -f ${COMPOSE_FILE} ps"
  prun "docker compose -f ${COMPOSE_FILE} ps --format json || true"
  prun "docker compose -f ${COMPOSE_FILE} images || true"
else
  echo "ERROR: Compose file missing or unreadable even with privilege helper: ${COMPOSE_FILE}"
  prun "find ${KX_ROOT}/instances/${INSTANCE_ID} -maxdepth 5 -type f -printf '%s bytes %p\n' 2>/dev/null | sort || true"
fi

section "REMOTE 10. Runtime env and host/domain propagation"
prun "find ${KX_ROOT}/instances/${INSTANCE_ID} -maxdepth 4 -type f \( -name '*.env' -o -name '*env*' -o -name '*.json' -o -name '*.yml' -o -name '*.yaml' \) -printf '%s bytes %TY-%Tm-%Td %TH:%TM %p\n' 2>/dev/null | sort"

prun "grep -RInE 'KX_HOST|KX_HOST_ALIASES|KX_NETWORK_PROFILE|KX_EXPOSURE_MODE|KX_PUBLIC_MODE|DJANGO_ALLOWED_HOSTS|CSRF|NEXT_PUBLIC|TRAEFIK|${DOMAIN}|${DROPLET_HOST}|sslip' ${KX_ROOT}/instances/${INSTANCE_ID}/env ${KX_ROOT}/instances/${INSTANCE_ID}/state ${MANAGER_DIR}/.kx-ui 2>/dev/null | head -260"

section "REMOTE 11. Traefik files and routing clues"
prun "find ${KX_ROOT}/instances/${INSTANCE_ID} -maxdepth 6 -type f \( -iname '*traefik*' -o -iname '*dynamic*' -o -iname '*router*' -o -iname '*tls*' -o -iname '*acme*' \) -printf '%s bytes %p\n' 2>/dev/null | sort"
prun "find ${KX_ROOT} -maxdepth 7 -type f \( -iname 'acme.json' -o -iname '*cert*' -o -iname '*letsencrypt*' \) -printf '%s bytes %p\n' 2>/dev/null | sort"
prun "docker ps --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}' | grep -E 'traefik|frontend|django|media|postgres|redis|celery|flower|NAMES' || true"
...
if priv test -f "${CAPSULE_FILE}" 2>/dev/null; then
  prun "sha256sum ${CAPSULE_FILE}"
  prun "tar -tf ${CAPSULE_FILE} 2>/dev/null | head -200 || true"
  prun "tar -xOf ${CAPSULE_FILE} manifest.yaml 2>/dev/null | sed -n '1,220p' || true"
  prun "tar -xOf ${CAPSULE_FILE} docker-compose.capsule.yml 2>/dev/null | sed -n '1,260p' || true"
  prun "tar -xOf ${CAPSULE_FILE} profiles/public_vps.yaml 2>/dev/null | sed -n '1,220p' || true"
else
...
if priv test -f "${COMPOSE_FILE}" 2>/dev/null; then
  for svc in "${SERVICES[@]}"; do
    ...
    cid="$(priv docker compose -f "${COMPOSE_FILE}" ps -q "${svc}" 2>/dev/null)"
    if [ -n "${cid}" ]; then
      echo "container_id=${cid}"
      priv docker inspect --format 'name={{.Name}} status={{.State.Status}} running={{.State.Running}} restart={{.RestartCount}} exit={{.State.ExitCode}} oom={{.State.OOMKilled}} started={{.State.StartedAt}} finished={{.State.FinishedAt}} health={{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}} image={{.Config.Image}} ports={{json .NetworkSettings.Ports}}' "${cid}" 2>&1 | redact

      echo
      echo ">>> recent health log for ${svc}"
      priv docker inspect --format '{{if .State.Health}}{{range .State.Health.Log}}{{.Start}} exit={{.ExitCode}} output={{printf "%q" .Output}}{{"\n"}}{{end}}{{else}}no healthcheck{{end}}' "${cid}" 2>&1 | tail -50 | redact

      echo
      echo ">>> last 260 compose logs for ${svc}"
      priv docker compose -f "${COMPOSE_FILE}" logs --tail=260 "${svc}" 2>&1 | redact
    else
      echo "WARN: no container found for service ${svc}"
      priv docker compose -f "${COMPOSE_FILE}" ps "${svc}" 2>&1 | redact
    fi
  done
else
...
if priv test -f "${COMPOSE_FILE}" 2>/dev/null; then
  DJANGO_CID="$(priv docker compose -f "${COMPOSE_FILE}" ps -q django-api 2>/dev/null)"
  FRONTEND_CID="$(priv docker compose -f "${COMPOSE_FILE}" ps -q frontend-next 2>/dev/null)"
  TRAEFIK_CID="$(priv docker compose -f "${COMPOSE_FILE}" ps -q traefik 2>/dev/null)"
  POSTGRES_CID="$(priv docker compose -f "${COMPOSE_FILE}" ps -q postgres 2>/dev/null)"
  REDIS_CID="$(priv docker compose -f "${COMPOSE_FILE}" ps -q redis 2>/dev/null)"

  if [ -n "${DJANGO_CID}" ]; then
    prun "docker exec ${DJANGO_CID} sh -lc 'python --version; env | sort | grep -E \"DJANGO|DATABASE|REDIS|CELERY|KX_|ALLOWED|CSRF|HOST|DEBUG\"' "
    prun "docker exec ${DJANGO_CID} sh -lc 'python manage.py check --deploy 2>&1 | head -160' "
    prun "docker exec ${DJANGO_CID} sh -lc 'python manage.py showmigrations --plan 2>&1 | tail -120' "
  fi

  if [ -n "${FRONTEND_CID}" ]; then
    prun "docker exec ${FRONTEND_CID} sh -lc 'node --version 2>/dev/null || true; env | sort | grep -E \"NEXT|KX_|HOST|API|BACKEND\"' "
  fi

  if [ -n "${POSTGRES_CID}" ]; then
    prun "docker exec ${POSTGRES_CID} sh -lc 'pg_isready -U konnaxion -d konnaxion 2>&1 || pg_isready 2>&1 || true' "
  fi

  if [ -n "${REDIS_CID}" ]; then
    prun "docker exec ${REDIS_CID} sh -lc 'redis-cli ping 2>&1 || true' "
  fi

  if [ -n "${TRAEFIK_CID}" ]; then
    prun "docker exec ${TRAEFIK_CID} sh -lc 'traefik version 2>&1 || true; ls -R /etc/traefik 2>/dev/null | head -200' "
  fi
fi

section "REMOTE 18. Recent system errors"
prun "journalctl -p warning..alert -n 260 --no-pager || true"
prun "grep -RInE 'error|exception|traceback|fail|timeout|refused|bad gateway|502|503|504|connection reset|allowed host|csrf|migration|database|redis' ${KX_ROOT}/instances/${INSTANCE_ID}/logs ${KX_ROOT}/instances/${INSTANCE_ID}/state 2>/dev/null | tail -260 || true"
...
[System.IO.File]::WriteAllText($LocalRemoteSh, $RemoteScriptUnix, $Utf8NoBom)
...
Run-Step "6. SSH reachability" {
    ...
    $rc = $LASTEXITCODE
    if ($rc -eq 0) {
        $script:SshReachable = $true
    } else {
        $script:CriticalFailures.Add("SSH reachability failed (rc=$rc)")
        Write-Host "CRITICAL: SSH reachability failed (rc=$rc)"
    }
}

Run-Step "7. Copy remote diagnostic to /tmp" {
    if (-not $script:SshReachable) {
        Write-Host "SKIP: SSH is unavailable; remote copy not attempted."
        return
    }
    & scp.exe -i $SshKey `
        -P $SshPort `
    ...
    $rc = $LASTEXITCODE
    if ($rc -eq 0) {
        $script:RemoteCopySucceeded = $true
    } else {
        $script:CriticalFailures.Add("Remote diagnostic copy failed (rc=$rc)")
        Write-Host "CRITICAL: remote diagnostic copy failed (rc=$rc)"
    }
}

Run-Step "8. Execute remote diagnostic" {
    if (-not $script:RemoteCopySucceeded) {
        Write-Host "SKIP: remote diagnostic was not copied successfully."
        return
    }
    & ssh.exe -i $SshKey `
        -p $SshPort `
    ...
    $rc = $LASTEXITCODE
    if ($rc -eq 0) {
        $script:RemoteDiagnosticSucceeded = $true
    } else {
        $script:CriticalFailures.Add("Remote diagnostic execution failed (rc=$rc)")
        Write-Host "CRITICAL: remote diagnostic execution failed (rc=$rc)"
    }
}
...
$ExitCode = 0
if (-not $script:SshReachable) { $ExitCode = 20 }
elseif (-not $script:RemoteCopySucceeded) { $ExitCode = 21 }
elseif (-not $script:RemoteDiagnosticSucceeded) { $ExitCode = 22 }

Write-Host ""
if ($ExitCode -eq 0) {
    Write-Host "KX_DIAG_STATUS=OK"
    Write-Host "DONE"
} else {
    Write-Host "KX_DIAG_STATUS=TRANSPORT_FAILURE"
    foreach ($failure in $script:CriticalFailures) { Write-Host "- $failure" }
    Write-Host "DONE WITH CRITICAL TRANSPORT FAILURE"
}
Write-Host "Log file: $LogFile"
